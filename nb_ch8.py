"""Builder for the Chapter 8 companion notebook (maxima, minima, convexity).

Run:
    /opt/anaconda3/bin/python nb_ch8.py
emits ch8_convexity.ipynb in this folder. Validate by executing with nbconvert.
"""
import nbbuild as nbb

TITLE = "Maxima, Minima, and the Shape of a Function"
SUBTITLE = ("where a curve peaks, where it bottoms out, and why some bowls "
            "have no false floors")
BLURB = (
    "Three live pictures of the chapter's payoff: classify a critical point "
    "with *both* derivative tests while the curve is painted by its curvature, "
    "watch chords sit **above** and tangents sit **below** a convex graph, and "
    "drop gradient-descent balls into a convex bowl (all reach the one floor) "
    "versus a two-valley landscape (they split) — the keystone made visible: "
    "*a convex bowl has no false floors.*"
)


def build():
    cells = [
        nbb.header(8, TITLE, SUBTITLE, BLURB),
        nbb.setup_cell(),

        # ---------------------------------------------------------------
        # Widget 1 — Classify the critical point (both tests at once)
        # ---------------------------------------------------------------
        nbb.md(
            "## 1. Classify the critical point\n\n"
            "A **critical point** is where $f'(c)=0$ — a *candidate* for a max "
            "or a min, never a verdict on its own. Two tests deliver the "
            "verdict:\n\n"
            "- **First-derivative test:** watch the sign of $f'$ change across "
            "$c$. From $+$ to $-$ is a **maximum**; from $-$ to $+$ is a "
            "**minimum**; no change is **neither** (like $x^3$ at $0$).\n"
            "- **Second-derivative test:** read $f''(c)$. Positive $\\Rightarrow$ "
            "**minimum** (the curve cups upward), negative $\\Rightarrow$ "
            "**maximum** (it caps downward), zero $\\Rightarrow$ "
            "**inconclusive**.\n\n"
            "Pick a function. The curve is painted **green** where it is "
            "*concave up* ($f''>0$, a cup) and **red** where it is *concave "
            "down* ($f''<0$, a cap); hollow dots mark the **inflection points** "
            "where the two meet. Each critical point is flagged with what *both* "
            "tests conclude — and you can confirm they agree. *(Figures 8.2, "
            "8.3.)*"
        ),
        nbb.code(
        ...
        # ---------------------------------------------------------------
        # Widget 2 — Convexity checker: chords above, tangents below
        # ---------------------------------------------------------------
        nbb.md(
            "## 2. Convexity checker: chords above, tangents below\n\n"
            "Two of the **four faces of convexity** are pictures you can test by "
            "eye:\n\n"
            "- **Chord above graph** — the straight line between any two points "
            "on the graph lies *on or above* the curve.\n"
            "- **Tangent below graph** — the graph lies *on or above* every one "
            "of its tangent lines.\n\n"
            "A **convex** function ($f''\\ge 0$ everywhere) passes both, always. "
            "A non-convex one fails somewhere. Pick a function and slide the "
            "**seed** to draw fresh random chords (**orange**) and tangents "
            "(**red**). Each gets a $\\checkmark$ if it lands on the correct "
            "side and a $\\times$ if it doesn't — so the convex curves come up "
            "all checks, and the wavy one gives itself away. *(Faces 1 and 2.)*"
        ),
        nbb.code(
            "_W2 = {\n"
            "    'f(x) = x^2  (convex)':  dict(\n"
        ...
        # ---------------------------------------------------------------
        # Widget 3 — Convex vs non-convex descent (the keystone)
        # ---------------------------------------------------------------
        nbb.md(
            "## 3. Convex vs non-convex descent\n\n"
            "Here is the keystone of the whole book, made visible. Gradient "
            "descent only ever steps **downhill**: $x \\leftarrow x - \\eta\\, "
            "f'(x)$, with **learning rate** $\\eta$.\n\n"
            "- **Left — a convex bowl** $f(x)=x^2$. By the theorem, the one point "
            "where $f'=0$ is the *global* minimum, and there is nothing else to "
            "fall into. Balls dropped from *anywhere* (**purple**) all roll to "
            "the same single floor. **No false floors.**\n"
            "- **Right — a two-valley landscape** $f(x)=x^4-3x^2+x$. This is "
            "*non-convex*: two separate dips of different depth. Balls "
            "(**teal**) **split** — each falls into whichever valley is nearest "
            "and stops, even when a deeper one waits across the ridge. This is "
            "exactly the Chapter 9 lab where the starting point decided the "
            "outcome.\n\n"
            "Slide the **learning rate** and the **number of starts**. Watch the "
            "left panel always agree and the right panel disagree — that "
            "disagreement *is* non-convexity, and it is why \"is your problem "
            "convex?\" was, for decades, the first question in optimization."
        ),
        nbb.code(
        ...
        nbb.md(
            "---\n\n"
            "*Three readings of one chapter. The first sorts critical points "
            "with both tests while the curvature is painted on. The second tests "
            "convexity by eye — chords above, tangents below. The third makes "
            "the keystone unmistakable: on a convex bowl every downhill path "
            "ends at the same floor, while a non-convex landscape sends them to "
            "different valleys. That single difference — **no false floors** — "
            "is the line between optimization that is guaranteed to work and the "
            "non-convex world of Chapter 9, where training a model is part "
            "science and part art.*"
        ),
    ]
    nbb.build("ch8_convexity.ipynb", cells)
