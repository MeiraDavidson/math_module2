"""Builder for the Chapter 12 companion notebook (infinite series).

    cd notebooks && python3 nb_ch12.py

Emits notebooks/ch12_series.ipynb. The notebook is self-contained: its first
...
TITLE = "Adding Up Forever"
SUBTITLE = ("when does an infinite sum settle to a number — and where do "
            "Taylor's series really live?")
BLURB = (
    "Four live experiments on the last idea in the book — that adding "
    "infinitely many numbers is just taking a limit of *partial sums*. You "
    "will watch a series settle onto its limit or wander off to infinity, "
    "squeeze a sum against an area with the integral test, shade the interval "
    "where a power series is allowed to live, and add up a bouncing ball's "
    "whole path as a geometric series that lands on a finite total."
)


def build():
    cells = [
        nbb.header(12, TITLE, SUBTITLE, BLURB),
        nbb.setup_cell(),

        # ---- Widget 1: partial-sum plotter --------------------------------
        nbb.md(
            "## 1. Does it settle, or does it run away? — the partial sums\n\n"
            "An infinite series is nothing more exotic than the **limit of its "
            "partial sums** $S_N=\\sum_{n=1}^N a_n$ — the sequences of "
            "Chapter 1, now built from a running total. So the whole question "
            "of convergence is visible to the eye: plot $S_N$ against $N$ and "
            "*look*. A convergent series' dots crowd down onto a single height; "
            "a divergent one's climb or wander off without ever settling.\n\n"
            "Pick a series. The blue dots are the partial sums $S_N$. When the "
            "series converges, the red line marks the true limit and you watch "
            "the dots press onto it. The two cautionary tales sit side by side: "
            "$\\sum\\frac1{n^2}$ converges to $\\frac{\\pi^2}{6}$, but the "
            "harmonic series $\\sum\\frac1n$ — whose terms *also* shrink to "
            "zero — climbs forever. Terms dying is necessary, never sufficient."
        ),
        nbb.code(
        ...
        # ---- Widget 2: integral-test visualizer ---------------------------
        nbb.md(
            "## 2. Weigh the sum against an area — the integral test\n\n"
            "Here is the principle that earns this chapter its place at the "
            "*end* of a calculus book: a sum of positive, decreasing terms can "
            "be trapped against an **area**, using the very rectangle-and-curve "
            "picture you built for the integral. Draw each term $f(n)$ as a "
            "rectangle of width $1$ and height $f(n)$, and lay the rectangles "
            "against the curve $y=f(x)$. Because $f$ is decreasing, the sum and "
            "the integral $\\int_1^\\infty f$ differ by at most the first term "
            "— so they are **finite together or infinite together**.\n\n"
            "Pick $f$ and slide the number of terms. The blue curve is "
            "$y=f(x)$; the shaded rectangles are the series' terms. The read-out "
            "shows the running sum $\\sum f(n)$ and the running area "
            "$\\int_1^N f$ side by side. With $f=1/x^2$ the area converges (to "
            "$1$), so the sum is squeezed finite — that's why $\\sum 1/n^2$ "
            "converges. With $f=1/x$ the area is $\\ln N\\to\\infty$, dragging "
            "the harmonic sum up with it."
        ),
        nbb.code(
            ...
            "    ax.text(0.5, -0.24,\n"
            "            f'running sum  \\u03a3\\u2099\\u208c\\u2081^{N} f(n) = {partial_sum:.4f}      '\n"
        ...
        # ---- Widget 3: radius-of-convergence explorer ---------------------
        nbb.md(
            "## 3. Where a power series is allowed to live — the radius $R$\n\n"
            "A power series $\\sum c_n x^n$ is a Taylor series viewed as a sum "
            "that converges only for some $x$. The fundamental fact: there is a "
            "**radius of convergence** $R$ — the series converges for all "
            "$|x|<R$ and diverges for all $|x|>R$. The tool that finds it is the "
            "**ratio test**: form $\\left|c_n/c_{n+1}\\right|$, and its limit "
            "*is* $R$, because the series behaves geometrically with ratio "
            "$\\approx |x|/R$.\n\n"
            "Pick a power series. The plot shows $\\left|c_n/c_{n+1}\\right|$ "
            "settling onto $R$ (red line), and the green band on the $x$-axis is "
            "the interval $(-R,R)$ where the series genuinely equals its "
            "function. Watch $e^x$ fill the *whole line* ($R=\\infty$, the "
            "ratio marching off the top) while $\\frac1{1-x}$ stops dead at "
            "$\\pm 1$ — *that* is the wall at $\\pm1$ from Chapter 7, now derived "
            "rather than drawn."
        ),
        nbb.code(
            "import math\n"
            ...
            "}\n"
            "\n"
            "def _radius(series='e^x = \\u03a3 x\\u207f/n!   (R = \\u221e)', terms=12):\n"
        ...
        # ---- Widget 4: geometric ball animation ---------------------------
        nbb.md(
            "## 4. The bouncing ball — a geometric series with a finite total\n\n"
            "Here is the geometric series made physical. Drop a ball; each "
            "bounce returns it to a fixed fraction $r$ of its previous height. "
            "It bounces *infinitely many times* — yet the total distance it "
            "travels is **finite**, because the heights form a geometric "
            "series. Starting from height $h_0$, the ball falls $h_0$, then "
            "rises and falls $h_0 r$, then $h_0 r^2$, and so on:\n\n"
            "$$D = h_0 + 2h_0 r + 2h_0 r^2 + \\cdots = h_0\\,\\frac{1+r}{1-r}.$$\n\n"
            "Watch the bounces shrink. The read-out tracks the **running total** "
            "climbing toward that closed-form sum — infinitely many bounces, but "
            "the distance lands on a single finite number. (Here $h_0=1$ and "
            "$r=0.6$, so $D = \\frac{1.6}{0.4} = 4$.)"
        ),
        nbb.code(
    ...
    nbb.build("ch12_series.ipynb", cells)
